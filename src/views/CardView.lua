...
-- @todo - integrate animation colors into config.ColorRegistry
local mistakeRed = {}
...
-- @todo add animation colors to registry
function CardView.drawFlashingRedCard(anim)
    Logger.trace("CardView","drawFlashingRedCard")

    local flashIntensity = anim.flashIntensity or 0

    -- Get the complementary color as a base
    local baseR, baseG, baseB, baseA = unpack(cardColors.BACKGROUND)
    ...
    -- Draw red border with intensity
    -- local borderIntensity = 0.2 + flashIntensity * 0.8
    -- love.graphics.setColor(1, borderIntensity, borderIntensity)
    -- love.graphics.setLineWidth(4)
