...
function MenuScene.enter()
    Logger.trace("MenuScene", "Entering main menu scene")
    MenuView.initialize()
end

function MenuScene.exit()
    Logger.trace("MenuScene", "Exiting menu scene")
end
...
function MenuScene.onKeyPressed(key)
    Logger.trace("MenuScene", "Menu scene handling key: %s", key)
    if key == "escape" then
        Logger.info("MenuScene", "Escape key pressed. Quitting game")
        love.event.quit()
    end
end
function MenuScene.onMousePressed(x, y, button)
    Logger.trace("MenuScene", "Menu scene handling mouse press: (%d, %d) button %d", x, y, button)
    if button == 1 then -- Left mouse button
        if MenuView.isClassicButtonClicked(x, y) then
            Logger.info("MenuScene", "Classic mode button clicked")
        ...
        elseif MenuView.isRogueButtonClicked(x, y) then
            Logger.info("MenuScene", "Rogue mode button clicked")
